...
data.to_csv("outputs/data_countries_n.csv", sep="\t")
...
# k means clustering
clusters_kmeans = KMeans(n_clusters=10, random_state=0).fit(data_t)
...
''' data = data.T
data = (data - data.min()) / (data.max() - data.min())
data.to_csv("outputs/data_tags_n.csv", sep="\t")
pca = PCA(n_components=2).fit(data)
data_t = pca.transform(data)
np.savetxt("outputs/pca_tags_t.csv", data_t, delimiter="\t")
...
data.to_csv("outputs/pca_tags.csv", sep="\t") '''
